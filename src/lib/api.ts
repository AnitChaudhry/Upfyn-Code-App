...
export const api = {
  ...
  async runShellCommand(command: string, cwd: string): Promise<{ success: boolean; exit_code: number | null; stdout: string; stderr: string }> {
    return apiCall("run_shell_command", { command, cwd });
  },
  ...
  async saveApiKey(provider: string, apiKey: string): Promise<void> {
    return apiCall("save_api_key", { provider, apiKey });
  },

  /** Get stored API key for a provider (returns raw key) */
  async getApiKey(provider: string): Promise<string | null> {
    return apiCall("get_api_key", { provider });
  },

  /** Delete API key for a provider */
  async deleteApiKey(provider: string): Promise<void> {
    return apiCall("delete_api_key", { provider });
  },

  /** List all providers with their configuration status */
  async listProviders(): Promise<ProviderInfo[]> {
    return apiCall("list_providers");
  },

  /** Test an API key by making a lightweight request */
  async testApiKey(provider: string, apiKey: string): Promise<boolean> {
    return apiCall("test_api_key", { provider, apiKey });
  },

  /** List available models for configured providers */
  async listModels(): Promise<ModelInfo[]> {
    return apiCall("list_models");
  },

  /** Start a streaming BYOK chat — returns sessionId for event listening */
  async byokChat(
    provider: string,
    model: string,
    ...
    return apiCall("byok_chat", { provider, model, messages, systemPrompt });
  },

  /** Non-streaming BYOK chat — returns complete response */
  async byokChatSync(
    provider: string,
    model: string,
    messages: ChatMessage[],
    ...
    return apiCall("byok_chat_sync", { provider, model, messages, systemPrompt });
  },
