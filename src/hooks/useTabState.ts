...
interface UseTabStateReturn {
  ...
  createCanvasTab: () => string | null;
  createTerminalTab: () => string;
  createCodeEditorTab: (filePath?: string) => string;
  createFileTreeTab: () => string | null;
  createGitTab: () => string | null;
  createWorkflowsTab: () => string | null;
  createTaskMasterTab: () => string | null;
  createBrowserTab: () => string | null;
  createBYOKChatTab: () => string;
...
export const useTabState = (): UseTabStateReturn => {
  ...
  // Upfyn tab creation methods
  const createCanvasTab = useCallback((): string | null => {
    const existingTab = tabs.find(tab => tab.type === 'canvas');
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }
  ...
  const createTerminalTab = useCallback((): string => {
    return addTab({
      type: 'terminal',
      title: 'Terminal',
      status: 'idle',
  ...
  const createCodeEditorTab = useCallback((filePath?: string): string => {
    const title = filePath ? filePath.split('/').pop() || filePath.split('\\').pop() || 'Editor' : 'Editor';
    return addTab({
      type: 'code-editor',
      title,
  ...
  const createFileTreeTab = useCallback((): string | null => {
    const existingTab = tabs.find(tab => tab.type === 'file-tree');
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }
  ...
  const createGitTab = useCallback((): string | null => {
  ...
  const createWorkflowsTab = useCallback((): string | null => {
    const existingTab = tabs.find(tab => tab.type === 'workflows');
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }
  ...
  const createTaskMasterTab = useCallback((): string | null => {
    const existingTab = tabs.find(tab => tab.type === 'task-master');
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }
  ...
  const createBrowserTab = useCallback((): string | null => {
    const existingTab = tabs.find(tab => tab.type === 'browser');
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }
  ...
  const createBYOKChatTab = useCallback((): string => {
    return addTab({
      type: 'byok-chat',
      title: 'AI Chat',
      status: 'idle',
