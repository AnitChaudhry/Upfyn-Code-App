...
import { DesktopAuthProvider } from "@/contexts/DesktopAuthContext";
...
import { AuthProvider as UpfynAuthProvider } from "@/upfyn/contexts/AuthContext";
import { WebSocketProvider as UpfynWebSocketProvider } from "@/upfyn/contexts/WebSocketContext";
import { RelayProvider as UpfynRelayProvider } from "@/upfyn/contexts/RelayContext";
import { ToastProvider as UpfynToastProvider } from "@/upfyn/contexts/ToastContext";
...
function AppContent() {
  ...
  const { createClaudeMdTab, createSettingsTab, createUsageTab, createMCPTab, createAgentsTab, createCanvasTab, createTerminalTab, createCodeEditorTab, createFileTreeTab, createGitTab, createWorkflowsTab, createTaskMasterTab, createBrowserTab, createBYOKChatTab } = useTabState();
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <CustomTitlebar
        ...
        onCanvasClick={() => createCanvasTab()}
        onTerminalClick={() => createTerminalTab()}
        onEditorClick={() => createCodeEditorTab()}
        onFilesClick={() => createFileTreeTab()}
        onGitClick={() => createGitTab()}
        onWorkflowsClick={() => createWorkflowsTab()}
        onTasksClick={() => createTaskMasterTab()}
        onBrowserClick={() => createBrowserTab()}
        onBYOKChatClick={() => createBYOKChatTab()}
      />
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (enabled) {
          // keep intro visible briefly, then fade out
          timer = window.setTimeout(() => setShowIntro(false), 800);
        } else {
          // user disabled intro: hide immediately
          setShowIntro(false);
        }
      } catch (err) {
        timer = window.setTimeout(() => setShowIntro(false), 800);
