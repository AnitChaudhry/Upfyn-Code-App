...
import { IS_PLATFORM, WS_BACKEND_URL } from '../constants/config';
...
function Shell({ selectedProject, selectedSession, initialCommand, isPlainShell = false, onProcessComplete, minimal = false, autoConnect = false, shellType = null }) {
  ...
  const connectWebSocket = useCallback(async () => {
    ...
    try {
      ...
      // Cookie is sent automatically on WebSocket upgrade — no token in URL needed
      // Use VITE_WS_URL for split deployment (Vercel + Railway), same as chat WebSocket
      if (WS_BACKEND_URL) {
        const base = WS_BACKEND_URL.replace(/^http/, 'ws');
        wsUrl = `${base}/shell`;
      } else {
      ...
      // Connection timeout — if not connected within 15s, give up with error
      const connectTimeout = setTimeout(() => {
        if (ws.current && ws.current.readyState !== WebSocket.OPEN) {
          ws.current.close();
          setIsConnecting(false);
          setConnectionError('Connection timed out. Check your relay connection (uc connect).');
        }
      }, 15000);
      ...
      ws.current.onmessage = (event) => {
        try {
          ...
          if (data.type === 'output') {
            ...
            // Server-sent error (relay down, auth failed, etc.)
      ...
      ws.current.onclose = (event) => {
        ...
        // Show error if closed unexpectedly (not by user disconnect)
        if (event.code === 1006 || event.code === 4001) {
          setConnectionError('Connection lost. Your local machine may be offline.');
        } else if (event.code === 4003 || event.code === 1008) {
      ...
      ws.current.onerror = () => {
        ...
        setIsConnected(false);
        setIsConnecting(false);
        setConnectionError('Failed to connect. Check that your relay is running (uc connect).');
      };
    } catch (error) {
      setIsConnected(false);
      setIsConnecting(false);
        ...
        {isInitialized && !isConnected && !isConnecting && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-90 p-4">
            <div className="text-center max-w-sm w-full">
              {connectionError && (
                <div className="mb-4 px-4 py-2 bg-red-900/40 border border-red-500/30 rounded-lg">
              ...
              >
                ...
                <span>{connectionError ? t('shell.actions.retry', 'Retry') : t('shell.actions.connect')}</span>
              </button>
