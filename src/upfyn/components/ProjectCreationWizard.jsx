...
const ProjectCreationWizard = ({ onClose, onProjectCreated }) => {
  ...
  const [browserDrives, setBrowserDrives] = useState([]); // Windows drives from relay (e.g., ['C:', 'D:'])
  ...
  const handleCreate = async () => {
    ...
    try {
      if (workspaceType === 'new' && githubUrl) {
        ...
        // Cookie is sent automatically by EventSource — no token param needed
  ...
  const loadBrowserFolders = async (path) => {
    try {
      ...
      // Capture Windows drives from relay response (sent by CLI on Windows)
      ...
      {showFolderBrowser && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] border border-gray-200 dark:border-gray-700 flex flex-col">
            ...
            {/* Windows Drives — shown when relay reports drives (e.g., C:, D:, E:) */}
            {browserDrives.length > 0 && (
              <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-medium text-gray-500 dark:text-gray-400 mr-1">Drives:</span>
                  {browserDrives.map((drive) => {
                    const driveName = typeof drive === 'string' ? drive : drive.name || drive.path;
                    const drivePath = typeof drive === 'string' ? `${drive}\\` : drive.path || `${drive.name}\\`;
                    return (
                      <button
                        key={driveName}
                        onClick={() => navigateToFolder(drivePath)}
                        className={`px-3 py-1.5 text-sm font-mono font-medium rounded-md border transition-colors ${
                          browserCurrentPath.startsWith(driveName)
