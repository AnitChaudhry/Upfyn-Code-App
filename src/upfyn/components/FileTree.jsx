...
function FileTree({ selectedProject, onFileOpen, activeFilePath = null }) {
  ...
  // Auto-expand parent dirs when activeFilePath changes and scroll into view
  useEffect(() => {
    if (!activeFilePath || files.length === 0) return;

    // Normalize path separators
    const normalized = activeFilePath.replace(/\\/g, '/');

    // Find all parent directory paths for the active file
    const findParentPaths = (items, targetPath, ancestors = []) => {
      for (const item of items) {
        const itemPath = (item.path || '').replace(/\\/g, '/');
        if (item.type === 'directory' && item.children) {
          const newAncestors = [...ancestors, item.path];
          // Check if target is a direct child or deeper
          if (normalized.startsWith(itemPath + '/')) {
            const found = findParentPaths(item.children, targetPath, newAncestors);
            if (found) return found;
          }
        }
        if (item.type !== 'directory' && itemPath === normalized) {
          return ancestors;
        }
      }
      return null;
    ...
    const parentPaths = findParentPaths(files, normalized);
    ...
    // Scroll active file into view after DOM updates
  ...
  const renderFileTree = (items, level = 0) => {
    return items.map((item) => {
      ...
      const isActive = !isDir && activeFilePath && (item.path || '').replace(/\\/g, '/') === activeFilePath.replace(/\\/g, '/');

      return (
        <div key={item.path} className="select-none">
          <div
            ref={isActive ? activeFileRef : undefined}
            className={cn(
              ...
              !isActive && isDir && isOpen && 'border-l-2 border-primary/30',
              !isActive && isDir && !isOpen && 'border-l-2 border-transparent',
              !isActive && !isDir && 'border-l-2 border-transparent',
            )}
          ...
          >
            ...
            <span className={cn(
              ...
              isActive ? 'font-medium text-primary' : isDir ? 'font-medium text-foreground' : 'text-foreground/90'
            )}>
              {item.name}
            </span>
          </div>
      ...
      return (
        <div key={item.path} className="select-none">
          <div
            ref={isActive ? activeFileRef : undefined}
            className={cn(
              'group grid grid-cols-1 sm:grid-cols-12 gap-0 sm:gap-2 py-[3px] pr-2 hover:bg-accent/60 cursor-pointer items-center rounded-sm transition-colors duration-100',
              isActive && 'bg-primary/10 border-l-2 !border-primary font-medium',
              !isActive && isDir && isOpen && 'border-l-2 border-primary/30',
              !isActive && isDir && !isOpen && 'border-l-2 border-transparent',
              !isActive && !isDir && 'border-l-2 border-transparent',
            ...
            onClick={() => handleItemClick(item)}
          >
            <div className="sm:col-span-5 flex items-center gap-1.5 min-w-0">
              {renderItemIcons(item)}
              <span className={cn(
                'text-[13px] leading-tight truncate',
                isActive ? 'font-medium text-primary' : isDir ? 'font-medium text-foreground' : 'text-foreground/90'
              ...
              {/* Inline size on mobile */}
              {item.type === 'file' && item.size != null && (
                <span className="sm:hidden text-[10px] text-muted-foreground/60 ml-auto flex-shrink-0">
                  {formatFileSize(item.size)}
                </span>
              )}
            </div>
            <div className="hidden sm:block sm:col-span-2 text-sm text-muted-foreground tabular-nums">
              {item.type === 'file' ? formatFileSize(item.size) : ''}
            </div>
            <div className="hidden sm:block sm:col-span-3 text-sm text-muted-foreground">
              {formatRelativeTime(item.modified)}
            </div>
            <div className="hidden sm:block sm:col-span-2 text-sm text-muted-foreground font-mono">
              {item.permissionsRwx || ''}
            </div>
          </div>
    ...
    <div className="h-full flex flex-col bg-background">
      ...
      {viewMode === 'detailed' && filteredFiles.length > 0 && (
        <div className="px-3 pt-1.5 pb-1 border-b border-border">
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-2 px-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70">
            <div className="sm:col-span-5">{t('fileTree.name')}</div>
            <div className="hidden sm:block sm:col-span-2">{t('fileTree.size')}</div>
            <div className="hidden sm:block sm:col-span-3">{t('fileTree.modified')}</div>
            <div className="hidden sm:block sm:col-span-2">{t('fileTree.permissions')}</div>
          </div>
        </div>
      )}
