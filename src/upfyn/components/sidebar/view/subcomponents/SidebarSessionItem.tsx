    ...
    <div className="group relative">
      ...
      <div className="hidden md:block">
        <Button
          variant="ghost"
          className={cn(
            'w-full justify-start px-2 py-2.5 h-auto min-h-[48px] font-normal text-left hover:bg-accent/50 transition-colors duration-200',
            isSelected && 'bg-accent text-accent-foreground',
          )}
          onClick={() => onSessionSelect(session, project.name)}
        >
          <div className="flex items-start gap-2 min-w-0 w-full pr-14">
            <SessionProviderLogo provider={session.__provider} className="w-3 h-3 mt-1 flex-shrink-0" />
            <div className="min-w-0 flex-1">
              <div className="text-xs font-medium truncate text-foreground leading-5">{sessionView.sessionName}</div>
              <div className="flex items-center gap-1 mt-0.5">
                <Clock className="w-2.5 h-2.5 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">
                  {formatTimeAgo(sessionView.sessionTime, currentTime, t)}
        ...
        {!sessionView.isCursorSession && (
          <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-200 bg-accent/80 backdrop-blur-sm rounded-md px-1 py-0.5">
            {editingSession === session.id && !sessionView.isCodexSession ? (
              ...
              <>
                {!sessionView.isCodexSession && (
                  <button
                    className="w-7 h-7 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 rounded flex items-center justify-center"
                    onClick={(event) => {
                      event.stopPropagation();
                ...
                <button
                  className="w-7 h-7 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/40 rounded flex items-center justify-center"
                  onClick={(event) => {
                    event.stopPropagation();
                    requestDeleteSession();
                  }}
