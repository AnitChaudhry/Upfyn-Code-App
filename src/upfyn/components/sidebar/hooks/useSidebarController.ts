...
export function useSidebarController({
  ...
  const confirmDeleteSession = useCallback(async () => {
    ...
    // Optimistic: remove from UI immediately, before the API call
  ...
  const confirmDeleteProject = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast(error.error || t('messages.deleteProjectFailed'), 'error');
      }
    } catch (error) {
      console.error('Error deleting project:', error);
      addToast(t('messages.deleteProjectError'), 'error');
    } finally {
