...
export default function ChatComposer({
  ...
  const { voiceEnabled, toggleVoice, isPlaying } = useVoiceOutput();
  ...
  const mobileFloatingClass = isInputFocused
    ? 'max-sm:fixed max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:z-50 max-sm:bg-background max-sm:shadow-[0_-4px_20px_rgba(0,0,0,0.12)] max-sm:border-t max-sm:border-border/30 max-sm:rounded-b-none'
    : '';

  return (
    <div className={`sticky bottom-0 z-30 bg-background p-2 sm:p-3 flex-shrink-0 pb-2 ${mobileFloatingClass}`}>
      {!hasQuestionPanel && (
      ...
      {!hasQuestionPanel && <form onSubmit={onSubmit as (event: FormEvent<HTMLFormElement>) => void} className="relative max-w-4xl mx-auto">
        ...
        <div
          {...getRootProps()}
          className={`relative bg-card/80 backdrop-blur-sm rounded-2xl shadow-sm border border-border/40 focus-within:shadow-md focus-within:border-primary/25 focus-within:ring-1 focus-within:ring-primary/10 transition-all duration-200 overflow-hidden ${
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <button
              ...
              className="absolute left-1.5 top-1/2 transform -translate-y-1/2 w-9 h-9 flex items-center justify-center hover:bg-accent/60 rounded-xl transition-colors"
            ...
            <div className="absolute right-[88px] sm:right-[88px] top-1/2 transform -translate-y-1/2 flex items-center gap-0.5">
