...
import { useVoiceIntentRouter } from '../../../hooks/useVoiceIntentRouter';
...
function ChatInterface({
  ...
  useChatRealtimeHandlers({
    ...
    onReplaceTemporarySession,
    onNavigateToSession,
    onResponseComplete: speakTTS,
  });

  // Model selector — compute current model, options, and change handler for the input bar chip
  ...
  const currentModel = provider === 'claude' ? claudeModel : provider === 'codex' ? codexModel : cursorModel;
  ...
  }, [provider, setClaudeModel, setCodexModel, setCursorModel]);

  // Voice intent router — intercepts voice transcripts for navigation/action commands
  const { routeTranscript } = useVoiceIntentRouter({
    setActiveTab: setActiveTab || (() => {}),
    onChatInput: handleTranscript,
    ...
    <>
      <div className="h-full flex flex-col">
        ...
        <ChatComposer
          ...
          onTranscript={routeTranscript}
        />
      </div>

      <QuickSettingsPanel />
      <CommandConfirmDialog {...commandConfirmDialogProps} />
    </>
  );
}
