...
/** Detect system instructions, AGENTS.md, skills metadata, etc. that should never appear in chat */
export const isSystemNoise = (content: string): boolean => {
  ...
  return (
    ...
    /^#\s+AGENTS\.md instructions for/i.test(trimmed) ||
    trimmed.startsWith('# AGENTS.md') ||
    (trimmed.includes('## Skills') && trimmed.includes('### Available skills')) ||
    (trimmed.includes('### How to use skills') && trimmed.includes('Trigger rules:')) ||
    trimmed.startsWith('A skill is a set of local instructions') ||
    /^<system[_-]?prompt>/i.test(trimmed) ||
    trimmed.startsWith('## System Information') && trimmed.includes('Operating System:')
  );
};
...
export const convertCursorSessionMessages = (blobs: CursorBlob[], projectPath: string): ChatMessage[] => {
  ...
  for (let blobIdx = 0; blobIdx < blobs.length; blobIdx += 1) {
    ...
    if (text && text.trim() && !isSystemNoise(text)) {
...
export const convertSessionMessages = (rawMessages: any[]): ChatMessage[] => {
  ...
  rawMessages.forEach((message) => {
    if (message.message?.role === 'user' && message.message?.content) {
      ...
      const shouldSkip = !content || isSystemNoise(content);
    ...
    if (message.message?.role === 'assistant' && message.message?.content) {
      ...
      if (typeof message.message.content === 'string' && !isSystemNoise(message.message.content)) {
        converted.push({
          type: 'assistant',
          content: unescapeWithMathProtection(message.message.content),
          timestamp: message.timestamp || new Date().toISOString(),
