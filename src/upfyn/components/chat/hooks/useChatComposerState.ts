...
export function useChatComposerState({
  ...
  const { confirm: confirmDialog, dialogProps: commandConfirmDialogProps, ConfirmDialog: CommandConfirmDialog } = useConfirmDialog();
  ...
  const handleSubmit = useCallback(
    async (
      ...
      // Quick shell command: !command — execute directly without AI round-trip
      if (currentInput.trimStart().startsWith('!')) {
        const shellCommand = currentInput.trimStart().slice(1).trim();
        if (!shellCommand) return;

        const resolvedPath = selectedProject.fullPath || selectedProject.path || '';

        setChatMessages((prev) => [...prev, { type: 'user', content: currentInput, timestamp: new Date() }]);
        setIsLoading(true);
  ...
  const handleAbortSession = useCallback(() => {
    ...
    if (!targetSessionId) {
      // No concrete session ID available yet — nothing to abort
      return;
    }
