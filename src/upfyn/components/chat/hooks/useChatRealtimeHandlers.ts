...
import { isSystemNoise as isSystemNoiseCheck } from '../utils/messageTransforms';
...
interface UseChatRealtimeHandlersArgs {
  ...
  onResponseComplete?: (lastAssistantText: string) => void;
}
  ...
  useEffect(() => {
    ...
    if (!shouldBypassSessionFilter) {
      ...
      if (latestMessage.sessionId !== activeViewSessionId) {
        ...
        // Message for different session — skip
        return;
      }
    }
    ...
    switch (latestMessage.type) {
      ...
      // Relay sessions: CLI reports the real session ID for --continue support
      case 'relay-session-id': {
        const cliId = (latestMessage as any).cliSessionId;
        if (cliId) {
          setCurrentSessionId(cliId);
          sessionStorage.setItem('pendingSessionId', cliId);
        }
        break;
      }

      // Session busy: another request is already processing (opencode pattern)
      case 'session-busy':
      case 'session-lock-denied': {
        clearLoadingIndicators();
        const errorText = (latestMessage as any).error || 'Session is busy. Wait for the current request to finish.';
        setChatMessages((prev) => [
          ...prev,
          { type: 'system' as const, content: errorText, timestamp: new Date().toISOString() },
        ]);
        break;
      }
      ...
      case 'cursor-system':
        try {
          ...
          // cursor-system parse error — ignore
        }
        break;
      ...
      case 'cursor-result': {
        ...
        try {
          ...
          // cursor-result parse error — ignore
        }
      ...
      case 'cursor-output':
        try {
          ...
          // cursor-output parse error — ignore
        }
        break;
      ...
      case 'claude-complete': {
        ...
        // Trigger voice output for the last assistant message
        if (onResponseComplete) {
          setChatMessages((prev) => {
            // Find the last assistant text message
            for (let i = prev.length - 1; i >= 0; i--) {
              if (prev[i].type === 'assistant' && prev[i].content && !prev[i].isToolUse) {
                onResponseComplete(prev[i].content || '');
                break;
              }
            }
          ...
          switch (codexData.itemType) {
            case 'agent_message':
              if (codexData.message?.content?.trim()) {
                ...
                // Filter out system instructions, AGENTS.md dumps, skill definitions, etc.
                if (!isSystemNoiseCheck(content)) {
                  setChatMessages((previous) => [
                    ...previous,
                    {
                      type: 'assistant',
            ...
            default:
              // Unhandled codex item type — skip
          }
        }
      ...
      case 'shell-exec-output': {
        setIsLoading(false);
        setCanAbortSession(false);
        setClaudeStatus(null);
        const shellOutput = (latestMessage as any).output || '(no output)';
        const shellExitCode = (latestMessage as any).exitCode;
        const shellStatus = (latestMessage as any).status;
        const isError = shellStatus === 'error' || (shellExitCode && shellExitCode !== 0);
        setChatMessages((previous) => [
          ...previous,
          {
