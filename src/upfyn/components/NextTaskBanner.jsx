  ...
  const handleInitializeTaskMaster = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast(`Failed to initialize TaskMaster: ${error.message}`, 'error');
      }
    } catch (error) {
      console.error('Error initializing TaskMaster:', error);
      addToast('Error initializing TaskMaster. Please try again.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              {isLoading ? 'Initializing...' : 'Initialize TaskMaster AI'}
            </button>
            <button
              onClick={() => setShowTaskOptions(!showTaskOptions)}
              className="text-xs px-1.5 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 rounded transition-colors"
              title="More options"
            >
              {showTaskOptions ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            </button>
          </div>
        </div>
          ...
          <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800">
            ...
            <div className="flex flex-col gap-2">
              {!projectTaskMaster?.hasTaskmaster ? (
                <>
                <button
                  className="text-xs px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors text-left flex items-center gap-2 disabled:opacity-50"
                  onClick={handleInitializeTaskMaster}
                  disabled={isLoading}
                >
                  <Zap className="w-3 h-3" />
                  {isLoading ? 'Initializing...' : 'Initialize TaskMaster (Auto)'}
                </button>
                <button
                ...
                >
                  ...
                  Initialize via Terminal (Advanced)
                </button>
                </>
              ) : (
                <>
...
const CreateTaskModal = ({ currentProject, onClose, onTaskCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast('Error creating task. Please try again.', 'error');
    } finally {
