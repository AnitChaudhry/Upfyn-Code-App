...
/**
 * UsageDashboard component with async loading and skeleton UI
 */
    ...
    try {
      ...
      if (selectedDateRange === "all") {
        const [statsResult, sessionResult] = await Promise.all([
          withTimeout(api.getUsageStats(), 5000),
          withTimeout(api.getSessionStats(), 5000)
        ]);
        ...
        const [statsResult, sessionResult] = await Promise.all([
          withTimeout(api.getUsageByDateRange(
            startDate.toISOString(),
            endDate.toISOString()
          ), 5000),
          withTimeout(api.getSessionStats(
      ...
      // Show empty state instead of error for no data / timeout
      setStats({
        total_cost: 0,
        total_tokens: 0,
      ...
      setSessionStats([]);
      setError(err?.message === 'Request timed out' ? "Loading took too long. Data may still be indexing." : "No usage data found yet. Start a Claude session to see stats here.");
    } finally {
      setLoading(false);
    }
  }, [selectedDateRange, getCachedData, setCachedData]);

  // Load data on mount and when date range changes (deferred to avoid blocking paint)
  useEffect(() => {
    // Reset pagination when date range changes
    setProjectsPage(1);
    setSessionsPage(1);
    // Defer the data load so the UI paints the skeleton first
    const raf = requestAnimationFrame(() => {
      const timer = setTimeout(() => loadUsageStats(), 50);
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          ) : !stats || (stats.total_sessions === 0 && stats.total_tokens === 0) ? (
            <div className="flex items-center justify-center h-64 text-muted-foreground">
              <div className="text-center">
                <Calendar className="h-12 w-12 mx-auto mb-3 opacity-40" />
                <p className="text-sm">No usage data yet</p>
                <p className="text-xs mt-1 opacity-70">Start a Claude Code session to see usage stats here</p>
              </div>
            </div>
          ) : stats ? (
            <div className="space-y-6">
